...
namespace FinalProject_QUANLYKHO.View.ImportMaterialView
{
    public partial class ManagerExportBakeGo : Form
    {
        ...
        public void LoadDataIntoDataGridView()
        {
            ...
            invoices = invoceService.GetAllInvoicesByDateAndType(dateOfInvoiceValue, ContanstApp.XuatBanhDi);
        ...
        private void filterDataFollowExportDepartment(ExportDepartment selectedDepartment)
        {
            ...
            List<Invoice> invoicesPreview = invoices.FindAll(inv => inv.idBoPhanXuat == selectedDepartment.idBoPhanXuat);
        ...
        {
            FormAddInvoceExportBakeGo form = new FormAddInvoceExportBakeGo();
        ...
        private void dataGridViewInvoice_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0) // Kiểm tra xem ô được nhấp vào có hợp lệ không
            {
                ...
                switch (selectedColumn)
                {
                    case 4:
                        {
                            DetailBakeBill formDetailInvoice = new DetailBakeBill(invoices, invoice);
