...
{
    public partial class DetailBakeBillImport : Form
    {
        ...
        private List<InvoiceDetailBakeImport> invoiceDetailBakes = new List<InvoiceDetailBakeImport>();
        ...
        public DetailBakeBillImport(List<Invoice> invoices, Invoice invoice,Customer customer)
        {
        ...
        public void loadData()
        {
            ...
            invoiceDetailBakes = invoiceBakeService.GetAllInvoiceBakeImportByIdBill(invoice.idHoaDon);
            /*            invoiceDetailsChange = invoiceDetails.Select(invDT => invDT.Clone()).ToList();
            */
            calculatorTotal(invoiceDetailBakes);
            lbImportCus.Text = customer.tenKhachHang;

            foreach (InvoiceDetailBakeImport invDT in invoiceDetailBakes)
            {
        ...
        public void calculatorTotal(List<InvoiceDetailBakeImport> detailInvoices)
        {
        ...
        private void btnSaveEdit_Click(object sender, EventArgs e)
        {
            ...
            foreach (InvoiceDetailBakeImport invoiceDetailBake in invoiceDetailBakes)
            {
                invoiceBakeService.DeleteDetailInvoiceImportBakeOut(invoice.idHoaDon, invoiceDetailBake.idBanh);
            }
            bakes = bakeService.GetAll(false);
            bool checkErr = true;
            ...
            foreach (DataGridViewRow row in dataGridViewDetailInvoice.Rows)
            {
                ...
                invoceDetail.idKhachHang = customer.maKhachHang;
                if (row.Cells[4].Value == null)
                {
                    break;
                }
